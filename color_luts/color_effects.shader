...
uniform sampler2D cold_lut;
uniform sampler2D warm_lut;
uniform float lut_size = 16.0;

uniform float filter_alpha : hint_range(0, 1) = 1.0;
uniform float temperature : hint_range(-1.0, 1.0) = 0.0;

// combine/process all the LUTs here
vec3 computeLut(ivec2 uv) {
	vec3 base = texelFetch(lut, uv, 0).rgb;
	vec3 cold = texelFetch(cold_lut, uv, 0).rgb;
	vec3 warm = texelFetch(warm_lut, uv, 0).rgb;
	return mix(mix(base, warm, clamp(temperature, 0, 1)), cold, clamp(-temperature, 0, 1));
}
...
vec4 get_lut_mapping_trilinear(vec4 old_color){
	...
	vec3 lut_color_fff = convert_linear_to_srgb(computeLut(lut_color_fff_pos));
	vec3 lut_color_ffc = convert_linear_to_srgb(computeLut(lut_color_ffc_pos));
	vec3 lut_color_fcf = convert_linear_to_srgb(computeLut(lut_color_fcf_pos));
	vec3 lut_color_fcc = convert_linear_to_srgb(computeLut(lut_color_fcc_pos));
	vec3 lut_color_cff = convert_linear_to_srgb(computeLut(lut_color_cff_pos));
	vec3 lut_color_cfc = convert_linear_to_srgb(computeLut(lut_color_cfc_pos));
	vec3 lut_color_ccf = convert_linear_to_srgb(computeLut(lut_color_ccf_pos));
	vec3 lut_color_ccc = convert_linear_to_srgb(computeLut(lut_color_ccc_pos));
